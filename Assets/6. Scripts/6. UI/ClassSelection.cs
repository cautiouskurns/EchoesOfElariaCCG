...
using TMPro;
using System.Linq; // Add LINQ for better null checks
...
public class ClassSelection : MonoBehaviour
{
    ...
    [Header("Selected Class UI")] 
    [SerializeField] private Image[] selectedClassIcons;
    [SerializeField] private TextMeshProUGUI[] selectedClassNames;

    private CharacterClass[] selectedClasses = new CharacterClass[2]; // ✅ Store 2 selected classes
    ...
    public void SelectClass(int index)
    {
        ...
        if (currentSelectionIndex >= selectedClasses.Length)
        {
            Debug.LogWarning("[ClassSelection] ⚠️ Both classes have already been selected.");
            return;
        }
    ...
    {
        if (!selectedClasses.All(c => c != null))
        {
        ...
        {   
            ...
            Debug.Log("[ClassSelection] ✅ Both classes selected. Transitioning to overworld...");
            SceneManager.LoadScene(nextSceneName);
        }
        else
        {
            Debug.LogWarning("[ClassSelection] ⚠️ GameManager is missing! Proceeding anyway...");
            SceneManager.LoadScene(nextSceneName); // Optional: Continue even if GameManager is missing
        }
    }
